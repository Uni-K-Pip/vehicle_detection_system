...
long long json_to_int(const json & value)
{
  // is_number_unsigned() must be checked before is_number_integer():
  // nlohmann/json reports unsigned integers as true for both, and
  // get<long long>() on an unsigned value above LLONG_MAX silently wraps.
  if (value.is_number_unsigned()) {
    const auto u = value.get<unsigned long long>();
  ...
  if (value.is_number_float()) {
    ...
    // long long is exactly -2^63 .. 2^63 - 1. Both bounds are representable
    // as double, but static_cast<double>(LLONG_MAX) rounds up to 2^63, so
    // compare against the exclusive upper bound 2^63 (= -LLONG_MIN as double)
